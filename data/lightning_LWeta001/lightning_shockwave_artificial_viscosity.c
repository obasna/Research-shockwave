...
int main(void) {
  ...
  double c,Tout,Tin,K,m,R1,R2,Na,nu,gamma,deltax,deltat,time,rho2new,rho1new,u1new,u2new,p1new,p2new;
  double rhonew[xMAX+1],rho[xMAX+1],unew[xMAX+1],u[xMAX+1],pnew[xMAX+1],p[xMAX+1],T[xMAX+1];
  eta = 0.01;
  Na = 6.02214e23;
  m = 0.003 * 0.003 * 0.003 * 3.4;/*Horanyiの直径 + https://www.wakusei.jp/book/pp/2006/2006-1-03/2006-1-03.pdf*/
  R1 = 8.31451e7; /*気体定数*/
  R2 = R1 / m / Na;
  c = 3.4e4;/*音速*/
  gamma = 1.4;
  ...
  FILE *file;
	file = fopen("full-lightning-shockwave-rho-grid1000-eta001.dat","w");
  fprintf(file, "%lf ",0.0);
